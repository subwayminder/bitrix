...
$propAddr = \Ipolh\SDEK\option::get('pvzPicker');//определяем инпуты, куда писать адреса
...
if(!(count($arParams['CITIES'])==1 && $arResult['DELIVERY']['pickup'] == 'no')){
	...
	$arList['PVZ'] = CDeliverySDEK::weightPVZ((CDeliverySDEK::$orderWeight)?false:\Ipolh\SDEK\option::get('weightD'),$arList['PVZ']);
}

if($arList && count($arList)){
				...
				if($allCities || in_array($city,$arParams['CITIES'])){
				    foreach ($arPVZ as $code => $pvzInfo){
				        foreach ($unneededFields as $field){
                            if(array_key_exists($field,$pvzInfo)){
                                unset($arPVZ[$code][$field]);
                            }
                        }
