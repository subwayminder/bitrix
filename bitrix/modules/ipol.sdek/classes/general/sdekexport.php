...
class sdekExport extends sdekHelper{
	...
	public static function getOrderCity($id){ // используетс€ только в orderDetail.php
		if(!self::$optCity)
	...
	static function parseAddress(&$fields,$forse=false){
		...
		if(!$parsed && $forse && !($fields['street'] && $fields['house'] && $fields['flat'])){
			$arAdress=array();
