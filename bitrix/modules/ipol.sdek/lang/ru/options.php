...
$MESS['IPOLSDEK_OPT_realSeller'] = "Истинный продавец";
...
$MESS['IPOLSDEK_OPT_autoAddCities'] = "Автодобавление городов <a href='#' class='PropHint' onclick='return IPOLSDEK_setups.popup(\"pop-autoAddCities\", this);'></a>";
	// Прочее
...
$MESS ['IPOLSDEK_HELPER_addData'] = "В договоре с ИМ определяется условие, кто именно, ИМ или СДЭК, запрашивает у получателя расписание для доставки отправления. В случае, если ИМ самостоятельно запрашивает расписание, дату можно указать в соответствующем поле.<br>Модуль и API СДЭКа не проверяют на корректность выбранную дату доставки (только сравнивает с рассчетным временем доставки, возвращенным калькуляцией) - проверка поля возлагается на менеджера.";	
		// НДС
$MESS ['IPOLSDEK_HELPER_NDSUseCatalog'] = "В товары будут подставлены значения НДС, указанные в торговом каталоге, если их ставка - 0.20, 0.18, 0.10 или 0.00. Иные значения (в том числе и их отсутствие) будет заменено на НДС по умолчанию.";		
		// Свойства заявки
...
$MESS ['IPOLSDEK_FAQ_API_DESCR'] = "<p>Перед тем, как пользоваться функционалом модуля, необходимо подать заявку на предоставление учетной записи вашему магазину на веб-сервере СДЭК-а. Сделать это можно двумя способами.
<ul><li>Отправить письмо на адрес integrator@cdek.ru - придут уведомления из тикетной системы СДЭКа. В письме необходимо указать номер договора со СДЭК.</li>
<li>Зарегистрироваться под своим адресом электронной почты в системе технической поддержки СДЭК на <a href='https://jira.cdek.ru/servicedesk/customer/user/login?destination=portals' target='_blank'>сайте</a> и авторизоваться в ней. Для получения ключей интеграции необходимо создать запрос в <a href='https://jira.cdek.ru/servicedesk/customer/portal/4/create/27 'target='_blank'>этом разделе</a>.</li></ul>
В ответ придет информация об Account и Secure_password для доступа к API. Эти данные надо вбить в соответствующие поля.<br>
Эти доступы отличаются от доступов к личному кабинету!</p>
...
$MESS['IPOLSDEK_FAQ_OTHER_DESCR'] = "
    <div class='ipol_subFaq'>
        ...
        <div class='ipol_inst'>
            ...
            Учтите, что использовать Импорт с Местоположениями 2.0 НЕ РЕКОМЕНДУЕТСЯ! Используйте Автодобавление городов (включается в сервисных свойствах модуля).
            </p>
            <p>
