...
class CWDU_PageProps {
	const TableName = 'b_wdu_pageprops';
	 ...
	 */
	function OnEndBufferContent_Handler(&$Content) {
		global $APPLICATION;
		...
		if (in_array($CurPage, array('/bitrix/admin/public_file_property.php','/bitrix/admin/public_folder_edit.php'))) {
			...
			$Content = preg_replace_callback('#<tr style.*?>[\s]{0,}<td class="bx-popup-label bx-width30">(.*?):</td>[\s]{0,}<td>(.*?)</td>[\s]{0,}</tr>#is'.(self::IsUtf8()?'u':''),create_function(
				'$matches',
				'return CWDU_PageProps::WD_ModifyPagePropsDialogReplace($matches);'
			),$Content);
		}
		...
		if ($CurPage=='/bitrix/admin/settings.php' && $_GET['mid']=='fileman') {
			$JS = '
			var WD_Prop_Mess = {
				"POPUP_TITLE": "'.GetMessage('WDU_PAGEPROPS_POPUP_TITLE').'",
				"POPUP_HEAD_1": "'.GetMessage('WDU_PAGEPROPS_POPUP_HEAD_1').'",
				"POPUP_HEAD_2": "'.GetMessage('WDU_PAGEPROPS_POPUP_HEAD_2').'",
				"POPUP_SAVE": "'.GetMessage('WDU_PAGEPROPS_POPUP_SAVE').'",
				"POPUP_CANCEL": "'.GetMessage('WDU_PAGEPROPS_POPUP_CANCEL').'",
				"EDIT_LINK_TITLE": "'.GetMessage('WDU_PAGEPROPS_EDIT_LINK_TITLE').'",
				"SAVE_ERROR": "'.GetMessage('WDU_PAGEPROPS_SAVE_ERROR').'",
			};
			';
			$JS .= @file_get_contents($_SERVER['DOCUMENT_ROOT'].'/bitrix/js/'.WDU_MODULE.'/pageprops_fileman.js');
			$Content = str_ireplace('</head>','<script>'.$JS.'</script><style>'.$CSS.'</style></head>',$Content);
		}
	}
	...
	function GetTypes($Force=false) {
		...
		foreach($arDeclaredClasses as $ClassName) {
			if (stripos($ClassName,'CWDU_Pageprop_')===0) {
...
abstract class CWDU_PagePropsAll {
