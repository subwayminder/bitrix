...
class CWDU_Pageprop_SelectBox extends CWDU_PagePropsAll {
	...
	function ShowSettings($PropertyCode) {
		...
		?>
			...
			<div id="wd_pageprops_settings_selectbox">
						...
						<?foreach($arCurrentItem['DATA'] as $Key => $arOption):?>
							<tr class="adm-list-table-row"<?if($MainRow):?> data-main="Y" style="display:none"<?endif?>>
								...
								<td class="adm-list-table-cell align-center">
									<input type="button" value="<?=self::GetMessage('DELETE');?>" onclick="WD_Pageprops_Selectbox_DeleteRow(this);" />
								</td>
							</tr>
							<?$MainRow=false;?>
						<?endforeach?>
				...
				<hr/>
				<div>
					<input type="button" value="<?=self::GetMessage('ADD_ROW');?>" onclick="WD_Pageprops_Selectbox_AddRow();" />
				</div>
				<hr/>
			</div>
		<?
	...
	function ShowControls($arItem, $PropertyCode, $PropertyID, $PropertyValue, $SiteID) {
		...
		?>
			<select name="PROPERTY[<?=$PropertyID;?>][VALUE]">
				<option value=""><?=self::GetMessage('SELECT_OPTION_EMPTY');?></option>
				<?foreach($arItem['DATA'] as $Key => $arOption):?>
					<option value="<?=$Key;?>"<?if($Key==$PropertyValue):?> selected="selected"<?endif?>><?=$arOption['NAME'];?></option>
				<?endforeach?>
			</select>
		<?
