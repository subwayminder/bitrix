...
function WD_Prop_ShowPopup(Site, PropCode, PropName) {
	...
	$.ajax({
		url: '/bitrix/admin/wdu_pageprops.php?&lang='+phpVars['LANGUAGE_ID']+'&prop_site='+Site+'&prop_code='+PropCode+'&' + Math.random(),
		type: 'GET',
		data: '',
		success: function(HTML) {
			$(WD_Prop_Popup.PARTS.CONTENT_DATA).html(HTML);
			WD_Prop_Popup.Show();
			BX.closeWait();
		}
	});
	...
	$('#edit1_edit_table tr[id$=Propery]').each(function(){
		...
		$(this).find('td.adm-detail-content-cell-r > table').each(function(){
			if ($(this).find('input[name^=propstypes]').size()>0) {
				var Table = $(this);
				Table.find('tr').append('<td class="wdu_pageprops_customize"></td>').not('.heading').each(function(){
					var PropCode = $.trim($(this).find('td').eq(0).find('input[type=text]').eq(0).val());
					if (PropCode!='') {
						$(this).find('td input[type=text]').each(function(){
							$(this).attr('data-value',$(this).val());
						});
						$(this).find('.wdu_pageprops_customize').append('<a href="#" data-site="'+SiteID+'" title="'+WD_Prop_Mess['EDIT_LINK_TITLE']+'"></a>').find('a').click(function(Event){
